...
describe('HomeTopPanel', () => {
  ...
  it('builds country filters in server order and keeps 全部 as the unfiltered view', () => {
    ...
    expect(homeRegionOptions(nodes)).toEqual(['HK', 'JP', 'TW', 'CN'])
    ...
    expect(filterHomeNodesByRegion(nodes, 'TW').map((node) => node.id)).toEqual(['tw'])
    expect(filterHomeNodesByRegion(nodes, 'CN').map((node) => node.id)).toEqual(['cn'])
  ...
  it('turns configured desktop and mobile background images into safe shell variables', () => {
    expect(shellStyleForSettings(settings)).toEqual({
      ...
      '--zeno-card-shadow': '0 5px 30px rgba(0, 0, 0, 0.340)',
      '--zeno-card-filter': 'blur(18px) saturate(1.2)',
      '--zeno-overlay-shadow': '0 5px 30px rgba(0, 0, 0, 0.340)',
      '--zeno-modal-filter': 'none',
      '--zeno-modal-backdrop-filter': 'blur(18px) saturate(1.2)',
