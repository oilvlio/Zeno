...
describe('route stylesheet boundaries', () => {
  it('keeps heavyweight admin workspace styles out of the public entry stylesheet', () => {
    ...
    expect(baseStyles).not.toContain('.admin-inline-sort-row')
    ...
    expect(adminShellStyles).not.toContain('.admin-inline-sort-row')
    expect(adminStyles).toContain('.admin-modal-backdrop')
    expect(adminStyles).toContain('.admin-inline-sort-row')
...
describe('homepage and admin shell layout', () => {
  ...
  it('uses one unboxed desktop row and a no-scroll mobile two-column summary', () => {
    ...
    expect(styles).toMatch(/\.home-summary__metric\s*\{[^}]*min-height: 40px;[^}]*align-items: center;[^}]*justify-content: space-between;[^}]*gap: 6px;[^}]*padding: 0 6px;[^}]*font-size: 12px;[^}]*font-weight: 400;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.home-summary__metric-label\s*\{[^}]*justify-content: flex-start;[^}]*gap: 7px;[^}]*flex: none;[^}]*color: var\(--foreground\);[^}]*font-size: 11px;[^}]*font-weight: 400;[^}]*letter-spacing: \.01em;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.home-summary__metric-value > span\s*\{[^}]*color: var\(--foreground\);[^}]*font-size: 11px;[^}]*font-weight: 500;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.home-summary__metric-value--rate > span\s*\{[^}]*width: auto;[^}]*color: var\(--foreground\);[^}]*text-align: right;[^}]*\}/)
    expect(styles).not.toContain('.home-summary__metric-value--total strong { color: var(--summary-accent); }')
    expect(styles).not.toContain('.home-summary__metric--total .home-summary__metric-label { color: var(--summary-accent); }')
    ...
    expect(styles).toMatch(/\.home-summary__metric-icon\s*\{[^}]*width: 18px;[^}]*height: 18px;[^}]*color: var\(--summary-accent, var\(--orange\)\);[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-uptime\s*\{[^}]*grid-column: 3;[^}]*justify-self: end;[^}]*color: var\(--metric-green\);[^}]*font-size: 11px;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.metric-expiry,\s*\.metric-expiry\.is-safe,\s*\.metric-expiry\.is-soon,\s*\.metric-expiry\.is-urgent,\s*\.metric-expiry\.is-expired\s*\{\s*--metric-accent: var\(--metric-red\);\s*\}/)
    expect(styles).toContain('.metric-billing { --metric-accent: var(--metric-gold); }')
    expect(styles).toContain('.home-summary__metric--status { --summary-accent: var(--metric-green); }')
    expect(styles).toContain('.home-summary__metric--cost { --summary-accent: var(--metric-gold); }')
    expect(styles).toContain('.home-summary__metric--upload { --summary-accent: var(--metric-blue); }')
    expect(styles).toContain('.home-summary__metric--download { --summary-accent: var(--metric-purple); }')
    expect(styles).toMatch(/\.home-summary__status-dot\s*\{[^}]*width: 18px;[^}]*height: 18px;[^}]*display: inline-flex;[^}]*\}/)
    expect(styles).toMatch(/\.home-summary__status-dot::before\s*\{[^}]*width: 8px;[^}]*height: 8px;[^}]*background: var\(--summary-accent\);[^}]*\}/)
    ...
    expect(styles).toMatch(/\.home-summary__metric\s*\{\s*min-height: 34px; gap: 5px; padding: 0 6px;\s*\}/)
    expect(styles).toContain('.home-summary__metric--rate { gap: 5px; }')
    expect(styles).toContain('.home-summary__metric--rate > .home-summary__metric-label { width: calc(2em + 24px); }')
    expect(styles).toContain('.home-summary__metric-value--rate { gap: 4px; }')
    expect(styles).toContain('@media (max-width: 360px)')
    expect(styles).toContain('.home-summary__metric { gap: 3px; padding: 0 3px; font-size: 11px; }')
    expect(styles).toContain('.home-summary__metric-label { gap: 5px; }')
    ...
    expect(styles).toContain('.home-summary__metric--rate > .home-summary__metric-label { width: calc(2em + 23px); }')
  ...
  it('keeps Gaussian materials slider-driven on desktop paint layers and mobile cards', () => {
    const scope = ".kulin-shell[data-appearance='gaussian_blur']"
    expect(baseStyles).toContain(`${scope} :is(.home-top-card, .region-filter-bar, .state-panel, .kulin-node-card, .monitor-panel) {\n  box-shadow: var(--zeno-node-shadow, var(--zeno-card-shadow));`)
    expect(baseStyles).toContain(`${scope} :is(.home-top-card, .region-filter-bar, .state-panel, .kulin-node-card, .monitor-panel)::before {\n  -webkit-filter: var(--zeno-card-filter);\n  filter: var(--zeno-card-filter);`)
    expect(baseStyles).not.toContain('background: rgba(255, 255, 255, .24)')
    expect(baseStyles).not.toContain('background: rgba(15, 23, 42, .34)')
    expect(baseStyles).not.toContain('var(--page-surface) 88%')
  })

  it('keeps server metric text neutral with larger colorful icons', () => {
    expect(baseStyles).not.toContain('.metric-label { color: var(--muted); }')
    expect(baseStyles).toMatch(/\.metric-icon\s*\{[^}]*width: 18px;[^}]*height: 18px;/)
    expect(baseStyles).toMatch(/\.metric-icon svg\s*\{[^}]*width: 18px;[^}]*height: 18px;/)
    expect(baseStyles).not.toContain('.node-health-metric .metric-icon svg { width: 12px; height: 12px; }')
    expect(baseStyles).toContain('.metric-billing { --metric-accent: var(--metric-gold); }')
    expect(baseStyles).toContain('.node-metric:nth-child(even) { border-left: 1px solid var(--border); }')
    expect(baseStyles).toContain('.node-metric:nth-child(n + 3) { border-top: 1px solid var(--border); }')
  })
  ...
  it('enforces the whole-app single-surface allowlist', () => {
    ...
    expect(baseStyles).toMatch(/\.admin-modal\s*\{[^}]*backdrop-filter: var\(--zeno-modal-filter, blur\(var\(--zeno-card-blur\)\)\);[^}]*\}/)
    ...
    expect(baseStyles).toMatch(/@media \(max-width: 767px\)[\s\S]*?\.kulin-shell\[data-appearance='gaussian_blur'\] \.home-top-card,[\s\S]*?\.monitor-panel\s*\{[^}]*backdrop-filter: var\(--zeno-card-filter\);[^}]*\}/)
    ...
    expect(adminStyles).toMatch(/\.admin-modal-backdrop\s*\{[^}]*background: light-dark\(rgba\(15, 23, 42, \.1\), rgba\(2, 6, 23, \.24\)\);[^}]*backdrop-filter: var\(--zeno-modal-backdrop-filter, blur\(10px\)\);[^}]*\}/)
  })
  ...
  it('keeps the backend on the same outer-surface-only glass policy as the front page', () => {
    ...
    expect(styles).toContain('backdrop-filter: var(--zeno-modal-filter, blur(var(--zeno-card-blur)))')
    expect(adminShellStyles).toMatch(/\.admin-login-card\s*\{[^}]*background: transparent;[^}]*box-shadow: none;[^}]*\}/)
    expect(baseStyles).not.toContain('.monitor-panel,\n.admin-login-card,')
  ...
  it('keeps authenticated node management in compact lists without old card shells', () => {
    ...
    expect(adminStyles).toMatch(/\.admin-inline-sort-main\s*\{\s*min-width: 0;\s*display: flex;\s*flex-direction: row;\s*align-items: center;/)
    expect(adminStyles).toMatch(/@media \(max-width: 767px\)[\s\S]*?\.admin-inline-sort-main > strong\s*\{[^}]*min-width: 0;[^}]*flex: 1 1 auto;[^}]*\}/)
    expect(adminStyles).not.toMatch(/@media \(max-width: 767px\)[\s\S]*?\.admin-inline-sort-main\s*\{[^}]*flex-direction: column;/)
    ...
    expect(styles).toContain('.admin-inline-sort-row')
    expect(adminStyles).not.toContain('.admin-sort-modal')
    expect(adminStyles).not.toContain('.admin-sort-controls')
  ...
  it('simplifies backend chrome and gives secondary forms card-aligned sections', () => {
    ...
    expect(baseStyles).toMatch(/\.admin-modal\s*\{[^}]*backdrop-filter: var\(--zeno-modal-filter, blur\(var\(--zeno-card-blur\)\)\);[^}]*\}/)
    const modalBackdropRule = adminStyles.match(/^\.admin-modal-backdrop\s*\{([^}]*)\}/m)?.[1] ?? ''
    expect(modalBackdropRule).toContain('background: light-dark(rgba(15, 23, 42, .1), rgba(2, 6, 23, .24))')
    expect(modalBackdropRule).toContain('backdrop-filter: var(--zeno-modal-backdrop-filter, blur(10px))')
...
describe('Kulin-inspired color polish', () => {
  ...
  it('uses solid accent colors without gradients and keeps key identity emphasized', () => {
    ...
    expect(styles).toContain('.metric-label { min-width: 0; overflow: hidden; color: var(--foreground)')
    expect(styles).toContain('.node-title-line p { max-width: calc(100% - 23px); min-width: 0; flex: none; margin: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--foreground); font-size: 15px; font-weight: 600; line-height: 1.25')
    ...
    expect(styles).toContain('.node-metric strong { max-width: 100%; min-width: 0; flex: 1; overflow: hidden; color: var(--foreground); font-size: 11px; font-weight: 500; line-height: 1.15; text-align: right; text-overflow: ellipsis; white-space: nowrap;')
    expect(styles).toContain('.metric-billing { --metric-accent: var(--metric-gold); }')
    expect(styles).toContain('.node-health-metric {')
    expect(styles).toContain('--metric-accent: var(--metric-green);')
    expect(styles).toContain('.node-health-metric.health-loss { --metric-accent: var(--metric-orange); }')
    ...
    expect(styles).toMatch(/\.region-filter-buttons\s*\{[^}]*display: flex;[^}]*flex-wrap: wrap;[^}]*gap: 4px;[^}]*width: 100%;[^}]*\}/)
    expect(styles).toMatch(/\.region-filter-buttons > button\s*\{[^}]*height: 32px;[^}]*border: 1px solid transparent;[^}]*border-radius: var\(--radius-field\);[^}]*background: transparent;[^}]*\}/)
    expect(styles).toMatch(/\.home-currency-popover button\[data-active="true"\],\s*\.region-filter-buttons > button\[aria-pressed='true'\]\s*\{[^}]*border-color: var\(--control-selected-border\);[^}]*background: var\(--control-selected-background\);[^}]*color: var\(--control-selected-foreground\);[^}]*\}/)
    expect(styles).not.toMatch(/\.region-filter-buttons > button(?:\[aria-pressed='true'\])?::?after\s*\{/)
    expect(styles).toContain('.region-filter-buttons > button:focus-visible')
    expect(styles).not.toMatch(/\.region-filter-buttons::before\s*\{/)
    expect(baseStyles.slice(baseStyles.indexOf('.region-filter-buttons {'), baseStyles.indexOf('.region-all-text {'))).not.toContain('--slider-')
