...
describe('homepage and admin shell layout', () => {
  ...
  it('keeps server metric text in its icon color with larger matching icons', () => {
    expect(baseStyles).not.toContain('.metric-label { color: var(--muted); }')
    expect(baseStyles).toMatch(/\.metric-icon\s*\{[^}]*width: 16px;[^}]*height: 16px;/)
    expect(baseStyles).toMatch(/\.metric-icon svg\s*\{[^}]*width: 16px;[^}]*height: 16px;/)
