...
describe('homepage and admin shell layout', () => {
  ...
  it('uses one unboxed desktop row and a no-scroll mobile two-column summary', () => {
    ...
    expect(styles).toMatch(/\.node-usage-grid\s*\{[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[^}]*gap: 18px 14px;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.usage-row__icon\s*\{[^}]*width: 13px;[^}]*height: 13px;[^}]*\}/)
    expect(styles).toMatch(/\.node-footer-grid\s*\{[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[^}]*gap: 8px;[^}]*\}/)
    expect(styles).toMatch(/\.node-metric\s*\{[^}]*min-height: 38px;[^}]*flex-direction: row;[^}]*justify-content: space-between;[^}]*gap: 8px;[^}]*font-size: 11px;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-health-metric\s*\{[^}]*font-size: 11px;[^}]*\}/)
