...
describe('homepage and admin shell layout', () => {
  ...
  it('uses a structured homepage summary without duplicate overview cards', () => {
    ...
    expect(styles).toContain('grid-template-columns: minmax(250px, 1.3fr) repeat(2, minmax(0, .85fr))')
    ...
    expect(styles).toContain('.home-summary__network-rate strong')
    expect(styles).toContain('.home-summary__network-total')
    expect(styles).toContain('.home-currency-menu')
    expect(styles).toMatch(/\.home-currency-select\s*\{[^}]*display: inline-flex;[^}]*flex-wrap: nowrap;[^}]*white-space: nowrap;[^}]*\}/)
    expect(styles).not.toContain('.home-currency-select__flag')
    expect(styles).toContain('.home-currency-select__value')
    ...
    expect(styles).toMatch(/\.home-summary__status\s*\{[^}]*z-index: 3;[^}]*overflow: visible;[^}]*\}/)
    expect(styles).toMatch(/\.node-head\s*\{[^}]*min-height: 28px;[^}]*grid-template-columns: 72px minmax\(0, 1fr\) 72px;[^}]*gap: 0;[^}]*text-align: center;[^}]*\}/)
    expect(styles).toMatch(/\.node-os\s*\{[^}]*top: 50%;[^}]*width: 20px;[^}]*height: 20px;[^}]*transform: translateY\(-50%\);[^}]*\}/)
    expect(styles).toMatch(/\.node-title-line\s*\{[^}]*grid-column: 2;[^}]*display: grid;[^}]*grid-template-columns: 19px minmax\(0, auto\) 19px;[^}]*justify-content: center;[^}]*column-gap: 4px;[^}]*\}/)
    expect(styles).toContain(".node-title-line::after { content: ''; width: 19px; height: 1px; }")
    expect(styles).not.toContain('.node-dot')
    expect(styles).not.toContain('.node-title-copy')
    expect(styles).toMatch(/\.node-renewal-meta\s*\{[^}]*grid-column: 3;[^}]*justify-self: end;[^}]*flex-direction: column;[^}]*align-items: flex-end;[^}]*gap: 6px;[^}]*\}/)
    expect(styles).toMatch(/\.node-renewal-cost\.is-safe,\s*\.node-expiry\.is-safe\s*\{\s*color: var\(--green\);\s*\}/)
    expect(styles).toMatch(/\.node-renewal-cost\.is-soon,\s*\.node-expiry\.is-soon\s*\{\s*color: var\(--orange\);\s*\}/)
    expect(styles).toMatch(/\.node-renewal-cost\.is-urgent,[\s\S]*\.node-expiry\.is-expired\s*\{\s*color: var\(--red\);\s*\}/)
    ...
    expect(styles).toMatch(/\.home-summary__status-heading\s*\{[^}]*transform: translateY\(-3px\);[^}]*\}/)
    expect(styles).toMatch(/\.home-summary__status-body\s*\{[^}]*margin-top: 3px;[^}]*\}/)
    expect(styles).toContain('.home-summary__status { grid-column: 1 / -1; min-height: 84px; padding: 10px 12px; }')
    expect(styles).toContain('.home-summary__status-body { margin-top: 0; }')
    expect(styles).toMatch(/\.home-summary__status-cost\s*\{[^}]*flex-direction: row;[^}]*align-items: baseline;[^}]*gap: 6px;[^}]*\}/)
    expect(styles).toContain('.home-summary__status-cost { max-width: 58%; gap: 4px; }')
