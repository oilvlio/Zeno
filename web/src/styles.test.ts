...
describe('homepage and admin shell layout', () => {
  ...
  it('uses one unboxed desktop row and a no-scroll mobile two-column summary', () => {
    ...
    expect(styles).toMatch(/\.node-head\s*\{[^}]*min-height: 24px;[^}]*grid-template-columns: 56px minmax\(0, 1fr\) 56px;[^}]*text-align: center;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-status\s*\{[^}]*grid-column: 3;[^}]*justify-self: end;[^}]*display: inline-flex;[^}]*font-size: 12px;[^}]*\}/)
    expect(styles).toMatch(/\.node-status-dot\s*\{[^}]*width: 6px;[^}]*height: 6px;[^}]*border-radius: 50%;[^}]*\}/)
    expect(styles).toContain('.node-status.is-online { color: var(--green); }')
    expect(styles).toContain('.node-status.is-offline { color: var(--red); }')
    ...
    expect(styles).not.toContain('.node-specs')
    expect(styles).toMatch(/\.node-usage-grid\s*\{[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[^}]*gap: 0;[^}]*border: 1px solid var\(--border\);[^}]*border-radius: var\(--radius-field\);[^}]*\}/)
    expect(styles).toMatch(/\.usage-track\s*\{[^}]*height: 7px;[^}]*\}/)
    expect(styles).toMatch(/\.usage-row__detail\s*\{[^}]*display: flex;[^}]*justify-content: space-between;[^}]*gap: 8px;[^}]*\}/)
    expect(styles).toContain('.usage-row__detail span:first-child { flex: none; }')
    expect(styles).toMatch(/\.usage-row__detail span:last-child\s*\{[^}]*max-width: none;[^}]*flex: 1 1 auto;[^}]*text-align: right;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-footer-grid\s*\{[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[^}]*gap: 0;[^}]*border: 1px solid var\(--border\);[^}]*\}/)
    expect(styles).toMatch(/\.node-metric\s*\{[^}]*min-height: 38px;[^}]*flex-direction: row;[^}]*justify-content: space-between;[^}]*gap: 8px;[^}]*border: 0;[^}]*background: transparent;[^}]*font-size: 11px;[^}]*\}/)
    expect(styles).toContain('.node-metric:nth-child(2n) { border-left: 1px solid var(--border); }')
    ...
    expect(styles).toMatch(/\.node-usage\s*\{[^}]*flex: none;[^}]*align-content: start;[^}]*\}/)
    expect(styles).toMatch(/\.node-health-history\s*\{[^}]*display: grid;[^}]*gap: 0;[^}]*border: 1px solid var\(--border\);[^}]*\}/)
    expect(styles).toMatch(/\.node-health-metric\s*\{[^}]*border: 0;[^}]*background: transparent;[^}]*font-size: 11px;[^}]*\}/)
    expect(styles).toContain('.node-health-metric + .node-health-metric { border-top: 1px solid var(--border); }')
...
describe('state history layout', () => {
  ...
  it('keeps server-card usage tracks visible on white cards', () => {
    ...
    expect(styles).toContain('.usage-track { position: relative; width: 100%; height: 7px; overflow: hidden; border: 0;')
  })
})
