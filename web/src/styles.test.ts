...
describe('homepage and admin shell layout', () => {
  ...
  it('keeps route and admin workspace loading cards at one stable destination-aligned position', () => {
    ...
    expect(baseStyles).toMatch(/\.route-state-card,\s*\.admin-state-card\s*\{[^}]*min-height: 96px;[^}]*margin-top: 18px;[^}]*display: flex;[^}]*gap: 10px;[^}]*padding: 18px 20px;[^}]*border: 1px solid var\(--border\);[^}]*background: var\(--control-bg\);[^}]*\}/)
  ...
  it('uses one unboxed desktop row and a no-scroll mobile two-column summary', () => {
    ...
    expect(styles).toMatch(/\.node-footer-grid\s*\{[^}]*--node-footer-item-height: 36px;[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[^}]*gap: 6px;[^}]*\}/)
    expect(styles).toMatch(/\.node-metric\s*\{[^}]*min-height: var\(--node-footer-item-height\);[^}]*flex-direction: row;[^}]*justify-content: space-between;[^}]*gap: 4px;[^}]*border: 1px solid var\(--border\);[^}]*background: var\(--control-bg\);[^}]*font-size: 11px;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-usage\s*\{[^}]*margin-top: 20px;[^}]*flex: none;[^}]*grid-template-rows: auto auto;[^}]*align-content: start;[^}]*gap: 14px;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-health-metric\s*\{[^}]*min-height: var\(--node-footer-item-height\);[^}]*padding: 8px;[^}]*border: 1px solid var\(--border\);[^}]*background: var\(--control-bg\);[^}]*font-size: 11px;[^}]*\}/)
    ...
    expect(styles).toContain('.home-summary__metric-label { font-size: 10px; }')
  ...
  it('keeps general popovers unified and admin secondary popovers on the appearance-controlled surface', () => {
    ...
    expect(baseStyles).toMatch(/\.theme-menu-popover,\s*\.admin-select-popover,\s*\.admin-install-platforms\s*\{[^}]*background: var\(--zeno-popover-bg\);[^}]*backdrop-filter: blur\(var\(--zeno-popover-blur\)\) saturate\(1\.04\);[^}]*\}/)
    ...
    expect(adminStyles).toMatch(/\.admin-select-popover,\s*\.admin-date-popover,\s*\.admin-install-platforms\s*\{[^}]*--zeno-popover-blur: var\(--zeno-card-blur\);[^}]*\}/)
    expect(baseStyles).toMatch(/\.theme-menu-popover,\s*\.admin-select-popover,\s*\.admin-install-platforms\s*\{[^}]*backdrop-filter: blur\(var\(--zeno-popover-blur\)\) saturate\(1\.04\);[^}]*\}/)
    expect(adminStyles).toMatch(/\.admin-select-popover,\s*\.admin-install-platforms\s*\{[^}]*background: var\(--admin-secondary-surface\);[^}]*\}/)
    expect(adminStyles).toMatch(/\.admin-date-popover\s*\{[^}]*background: var\(--admin-secondary-surface\);[^}]*backdrop-filter: blur\(var\(--zeno-popover-blur\)\) saturate\(1\.04\);[^}]*\}/)
  ...
  it('keeps authenticated node management in compact lists without old card shells', () => {
    ...
    expect(adminStyles).toContain('.admin-list-row:hover { position: relative; z-index: 1; background: color-mix(in srgb, var(--blue) 4%, transparent);')
  ...
  it('simplifies backend chrome and gives secondary forms card-aligned sections', () => {
    ...
    expect(baseStyles).toMatch(/\.home-top-card\s*\{[^}]*backdrop-filter: blur\(var\(--zeno-card-blur\)\);[^}]*\}/)
    ...
    expect(adminStyles).toMatch(/\.admin-list-row\s*\{[^}]*position: relative;[^}]*border-radius: var\(--radius-card\);[^}]*background: transparent;[^}]*\}/)
...
describe('Kulin-inspired color polish', () => {
  it('keeps the overall shell and cards pure white instead of aurora/grey backgrounds', () => {
    expect(styles).toContain('--background: light-dark(#ffffff, #07111f)')
    expect(styles).toContain('--card: light-dark(#ffffff, rgba(15, 23, 42, 0.62))')
