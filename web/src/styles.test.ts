...
describe('homepage and admin shell layout', () => {
  ...
  it('keeps popovers opaque and blur-free so they never stack glass surfaces', () => {
    ...
    expect(baseStyles).toMatch(/\.theme-menu-popover,\s*\.admin-select-popover,\s*\.admin-install-platforms\s*\{[^}]*background: var\(--zeno-popover-bg\);[^}]*backdrop-filter: none;[^}]*\}/)
  ...
  it('enforces the whole-app single-surface allowlist', () => {
    ...
    expect(baseStyles).toMatch(/\.home-top-card\s*\{[^}]*background: var\(--page-surface\);[^}]*backdrop-filter: blur\(var\(--zeno-card-blur\)\);[^}]*\}/)
    expect(baseStyles).toMatch(/\.kulin-node-card,\s*\.monitor-panel,\s*\.admin-modal\s*\{[^}]*backdrop-filter: blur\(var\(--zeno-card-blur\)\);[^}]*\}/)
    expect(baseStyles).toMatch(/\.region-filter-bar\s*\{[^}]*background: var\(--page-surface\);[^}]*backdrop-filter: blur\(var\(--zeno-card-blur\)\);[^}]*\}/)
    expect(baseStyles).toMatch(/\.state-panel\s*\{[^}]*background: var\(--page-surface\);[^}]*backdrop-filter: blur\(var\(--zeno-card-blur\)\);[^}]*\}/)
    ...
    expect(adminShellStyles).toMatch(/\.admin-section-nav\s*\{[^}]*background: transparent;[^}]*box-shadow: none;[^}]*\}/)
  ...
  it('keeps backend mobile navigation and every secondary action footer in the content plane', () => {
    ...
    expect(styles).toContain('.admin-chrome-card .admin-section-nav { width: 100%; display: grid; grid-template-columns: repeat(5, minmax(0, 1fr))')
    expect(styles).toContain('.admin-chrome-card .admin-section-nav button { min-width: 0; min-height: 38px; padding: 0 2px; border-radius: var(--radius-field); }')
    expect(styles).toContain('.admin-chrome-card .admin-section-nav button:last-child { grid-column: auto; }')
...
describe('state history layout', () => {
  it('keeps the detail hero dense with Nezha-like inline facts, not nested metric cards', () => {
    ...
    expect(detailStyles).toMatch(/\.detail-fact\s*\{[^}]*border: 1px solid color-mix\(in srgb, var\(--border\) 62%, transparent\);[^}]*background: transparent;[^}]*\}/)
    expect(detailStyles).toMatch(/\.detail-range-row > button\s*\{[^}]*background: transparent;[^}]*\}/)
  ...
  it('keeps every detail-page control and nested card on the outer single blur layer', () => {
    expect(detailStyles).toMatch(/\.detail-top-card \.nav-icon-button,[\s\S]*?\.detail-top-card \.login-link\s*\{[^}]*background: transparent;[^}]*box-shadow: none;[^}]*\}/)
    expect(baseStyles).toMatch(/\.theme-menu-popover,[\s\S]*?background: var\(--zeno-popover-bg\);[^}]*backdrop-filter: none;[^}]*\}/)
    expect(detailStyles).toMatch(/\.detail-status-pill\s*\{[^}]*border: 1px solid currentColor;[^}]*background: transparent;[^}]*\}/)
    expect(detailStyles).toMatch(/\.detail-range-row > button\.is-active\s*\{[^}]*background: transparent;[^}]*box-shadow: none;[^}]*\}/)
    expect(detailStyles).toMatch(/\.peak-switch span\s*\{[^}]*border: 1px solid var\(--border\);[^}]*background: transparent;[^}]*\}/)
    expect(baseStyles).toMatch(/\.latency-target-grid button\[data-active='true'\]\s*\{[^}]*background: transparent;[^}]*box-shadow: none;[^}]*\}/)
    expect(baseStyles).toMatch(/\.latency-tooltip-card\s*\{[^}]*background: transparent;[^}]*box-shadow: none;[^}]*\}/)
  ...
  it('renders resource history as a compact chart grid like Kulin on desktop', () => {
    ...
    expect(baseStyles).toMatch(/\.resource-card\s*\{[^}]*background: transparent;[^}]*box-shadow: none;[^}]*\}/)
  ...
  it('keeps backdrop-filter cards stationary on hover when a background image is active', () => {
    expect(baseStyles).toContain(".kulin-shell[data-background='on'] .kulin-node-card:hover { transform: none; }")
  })
