...
describe('homepage and admin shell layout', () => {
  ...
  it('uses one unboxed desktop row and a no-scroll mobile two-column summary', () => {
    ...
    expect(styles).toMatch(/\.node-uptime\s*\{[^}]*grid-column: 3;[^}]*justify-self: end;[^}]*color: var\(--muted\);[^}]*font-size: 11px;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-usage-grid\s*\{[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[^}]*gap: 20px 18px;[^}]*padding: 3px 1px 4px;[^}]*\}/)
    expect(styles).toMatch(/\.usage-row\s*\{[^}]*min-height: 58px;[^}]*gap: 8px;[^}]*padding: 0;[^}]*\}/)
    expect(styles).toMatch(/\.usage-track\s*\{[^}]*height: 8px;[^}]*\}/)
    expect(styles).toMatch(/\.usage-row__detail\s*\{[^}]*display: block;[^}]*font-size: 11px;[^}]*text-align: left;[^}]*\}/)
    expect(styles).not.toContain('.usage-row__detail span:first-child')
    expect(styles).not.toContain('.usage-row__detail span:last-child')
    expect(styles).toMatch(/\.usage-row__meta \.usage-row__icon\s*\{[^}]*width: 15px;[^}]*height: 15px;[^}]*color: var\(--resource-accent\);[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-metric\s*\{[^}]*min-height: 31px;[^}]*flex-direction: row;[^}]*justify-content: space-between;[^}]*gap: 6px;[^}]*border: 1px solid var\(--border\);[^}]*background: var\(--control-bg\);[^}]*font-size: 10px;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-usage\s*\{[^}]*flex: none;[^}]*align-content: start;[^}]*gap: 10px;[^}]*\}/)
    expect(styles).toMatch(/\.node-health-history\s*\{[^}]*display: grid;[^}]*gap: 6px;[^}]*\}/)
    expect(styles).toMatch(/\.node-health-metric\s*\{[^}]*min-height: 31px;[^}]*border: 1px solid var\(--border\);[^}]*background: var\(--control-bg\);[^}]*font-size: 10px;[^}]*\}/)
    expect(styles).not.toContain('.node-health-metric + .node-health-metric')
    expect(styles).toMatch(/\.history-cell\s*\{[^}]*height: 10px;[^}]*\}/)
...
describe('state history layout', () => {
  ...
  it('keeps server-card usage tracks visible on white cards', () => {
    ...
    expect(styles).toContain('.usage-track { position: relative; width: 100%; height: 8px; overflow: hidden; border: 0;')
  })
})
...
describe('Kulin-inspired color polish', () => {
  ...
  it('uses solid accent colors without gradients and keeps key identity emphasized', () => {
    ...
    expect(styles).toContain('.node-metric strong { max-width: 100%; min-width: 0; flex: 1; overflow: hidden; color: var(--metric-accent); font-size: 10px; font-weight: 500; line-height: 1.1; text-align: right; text-overflow: ellipsis; white-space: nowrap;')
    expect(styles).toContain('.metric-billing { --metric-accent: var(--billing-gold); }')
