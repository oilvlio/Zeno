...
describe('homepage and admin shell layout', () => {
  ...
  it('uses one unboxed desktop row and a no-scroll mobile two-column summary', () => {
    ...
    expect(styles).toMatch(/\.usage-row__detail\s*\{[^}]*display: flex;[^}]*justify-content: space-between;[^}]*gap: 8px;[^}]*font-size: 10px;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-metric\s*\{[^}]*min-height: 38px;[^}]*flex-direction: row;[^}]*justify-content: space-between;[^}]*gap: 8px;[^}]*border: 1px solid var\(--border\);[^}]*background: var\(--control-bg\);[^}]*font-size: 11px;[^}]*\}/)
    ...
    expect(styles).toMatch(/\.node-usage\s*\{[^}]*flex: none;[^}]*align-content: start;[^}]*gap: 14px;[^}]*\}/)
    expect(styles).toMatch(/\.node-health-history\s*\{[^}]*display: grid;[^}]*gap: 8px;[^}]*\}/)
    expect(styles).toMatch(/\.node-health-metric\s*\{[^}]*padding: 8px;[^}]*border: 1px solid var\(--border\);[^}]*background: var\(--control-bg\);[^}]*font-size: 11px;[^}]*\}/)
